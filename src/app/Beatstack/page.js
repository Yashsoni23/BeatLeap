...
export default function Beatstack() {
    ...
    const getPlaylistsTracks = (id) => {
        if (accessToken) {
            ...
            spotifyWebAPI.getPlaylistTracks(id).then((data) => {
                setCurrentPlaylistsTrack(data.body.items.map((track) => {
                    return {
    ...
    const startLoop = () => {
        ...
        const tracksUri = currentPlaylistsTrack.reduce((acc, track) => {
                    ...
                    <div className="flex flex-col w-full sm:w-1/2 overflow-hidden h-full">
                        <div className="flex flex-col max-[450px]:justify-between relative bg-slate-900 w-full h-full overflow-hidden">
                            <div className="flex flex-col sm:flex-row sm:sticky z-20 top-0 bg-transparent backdrop-blur-md h-[30%] bg-gradient-to-b from-indigo-500  w-full">
                                ...
                                <h1 className='absolute -bottom-20 sm:bottom-0 pb-5 right-0 text-md sm:text-3xl left-[10%] sm:left-[35%]'>{currentPlaylist.name}</h1>
                            </div>
                                        ...
                                        return (
                                            <>
                                                <div key={playlist.id} className={`flex cursor-pointer flex-col sm:items-center justify- gap-4`} onClick={() => setCurrentPlaylist({
                                                    ...
                                                    <img src={playlist.image} className="sm:w-[100px] sm:h-[100px]  w-[125px] h-[125px] object-cover" />
                                                    <h1 className="text-white text-xs sm:text-sm font-bold w-[125px] sm:w-[100px]">{playlist.name}</h1>
                                                </div>
