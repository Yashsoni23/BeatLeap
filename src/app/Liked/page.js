...
import { BsFileArrowUp, BsFillPeopleFill, BsFillSaveFill } from "react-icons/bs"
...
export default function Beatstack() {
    ...
    useEffect(() => {
        if (accessToken) {
            ...
            spotifyWebAPI.getMyTopTracks({ limit: 50 }).then((data) => {
                setMyTopTracks(data.body.items.map((track) => {
                    return {
                        name: track.name,
                        id: track.id,
                ...
                setTrackLiabrary(data.body.items.map((track) => {
                    return {
                        name: track.name,
                        id: track.id,
                        image: track.album.images[0].url,
            ...
            spotifyWebAPI.getMySavedTracks({ limit: 50 }).then((data) => {
                setMySavedTracks(data.body.items.map((track) => {
                    return {
            ...
            spotifyWebAPI.getMyTopArtists({ limit: 50 }).then((data) => {
                setMyTopArtists(data.body.items.map((artist) => {
                    return {
                        name: artist.name,
                        id: artist.id,
                ...
                setLiabrary(data.body.items.map((album) => {
                    return {
                        name: album.name,
                        id: album.id,
            ...
            spotifyWebAPI.getArtistAlbums("377MjbmPO6WvxAs3a9ET1I", { limit: 50 }).then((data) => {
                setArtistAlbums(data.body.items.map((album) => {
                    return {
                        name: album.name,
                        id: album.id,
                        image: album.images[0].url,
    ...
    const getAlbumsFromArtist = (id) =>{
        if(id){
            spotifyWebAPI.getArtistAlbums(id, { limit: 50 }).then((data) => {
                setTrackLiabrary(data.body.items.map((album) => {
                    return {
                        name: album.name,
                        id: album.id,
                        image: album.images[0].url,
    ...
    const startLoop = () => {
        ...
        const tracksUri = trackLiabrary.reduce((acc, track) => {
                                ...
                                liabrary && liabrary.map((artist) => {
                                    ...
                                    return (
                                        <div className={`flex ${currentTrack?.uri === artist?.uri ? "bg-gradient-to-t from-indigo-500 to-violet-500" : "bg-inherit"} flex-col items-center   sm:gap-3`}>
                                            ...
                                            <h1 className="text-white w-full text-[.5rem] sm:text-xs max-[450px]:w-[60px] font-bold sm:w-[100px]">{artist.name}</h1>
                                        </div>
                                    )
                                }
                                )
                    ...
                    <div className="player fixed bottom-0 w-full z-50 ">
                            {
                                ...
                                <BsFileArrowUp className='text-2xl cursor-pointer' onClick={() => setLiabrary(MyTopTracks)} />
                                <BsFillPeopleFill className='text-2xl cursor-pointer' onClick={() => setLiabrary(MyTopArtists)} />
                                <BsFillSaveFill className='text-2xl cursor-pointer' onClick={() => setLiabrary(MySavedTracks)} />
                                <IoArrowRedo className='text-2xl cursor-pointer' onClick={()=>setTrackLiabrary(MyTopTracks)}/>
                        ...
                        <div className="float-right absolute bottom-18 sm:right-44 sm:bottom-2 z-[99] right-0 font-bold text-2xl p-5 white">
                            <HiOutlineCog6Tooth className='cursor-pointer' onClick={()=>setToggle(!toggle)} />
                                ...
                                trackLiabrary && trackLiabrary.map((track) => {
                                    ...
                                    return (
                                        <div className={`flex ${currentTrack.uri === track.uri ? "bg-gradient-to-t from-indigo-500 to-violet-500" : "bg-inherit"} p-2 cursor-pointer flex-col sm:flex-row items-center sm:justify-between  sm:gap-3`}
                                            ...
                                            <img src={track.image} className=" w-[60px] h-[60px] object-cover" />
                                            <h1 className="text-white w-full text-[.5rem] sm:text-sm max-[450px]:w-[60px] font-bold ">{track.name}</h1>
                                            {
