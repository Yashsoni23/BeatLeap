...
export default function Navbar() {
    ...
    return (
        <div className='flex sticky top-0 z-50 bg-gradient-to-t from-black to-gray-800 justify-center sm:justify-between sm:p-2 sm:px-10 w-screen h-max'>
            <div className="flex z-[99] max-[450px]:bg-gradient-to-t from-black to-gray-800 max-[450px]:w-full relative justify-between sm:justify-center  px-2 items-center">
                ...
                <h1 className='text-primary text-xl font-bold text-center cursor-pointer' onClick={handleLogout}>
                    BeatLeap
                </h1 >
                {
                    toggleNav?
                    <HiOutlineMenuAlt2 className=' flex sm:hidden  top-3 text-primary  text-3xl font-bold' 
                    onClick={()=>setToggleNav(!toggleNav)}/>
                    :
                    <HiOutlineMenuAlt2 className=' flex sm:hidden  top-3 text-primary  text-3xl font-bold' onClick={()=>setToggleNav(!toggleNav)} />
            ...
            {
                    toggleNav ?
                        <div className={`absolute ${toggleNav ? "slideInDown":"slideOutUp"}   shadow-2xl flex flex-col gap-4 top-14 w-full  z-40 rounded-2xl  bg-slate-700  `}>
                            ...
                            <div onClick={() => setToggle(!toggle)} className='flex cursor-pointer relative justify-center gap-3 items-center text-l-white p-2 rounded-full bg-gradient-to-t from-gray-900 to-[#2934D1]'><BiUserCircle />User <IoIosArrowDropdown className={toggle ? "rotate-180" : "rotate-0"} />
                                <div className={`absolute z-30  ${toggle ? "zoomInUp flex  flex-col justify-center items-center" : "hidden zoomOutDown"} cursor-default w-full  gap-4 top-11 shadow-2xl right-0 z-50 rounded-2xl  bg-slate-900 p-2 `}>
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <h3 className='text-white text-center text-xl'>{userInfo.name}</h3>
            ...
            <div className="lg:flex hidden justify-center text-primary font-bold gap-10 text-xl  items-center">
